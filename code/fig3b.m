...
for ERP_idx=1:4
    ...
    rel_peak(1,1) = mean(bootstrp(200,@ICC,peak1,{'A-1'}));
    ci_peak(:,1) = bootci(200,@ICC,peak1,{'A-1'});
    if ERP_idx<=3
        peak2 = peak_amp(:,[3,4]+(ERP_idx-1)*4);
        rel_peak(1,2) = mean(bootstrp(200,@ICC,peak2,{'A-1'}));
        ci_peak(:,2) = bootci(200,@ICC,peak2,{'A-1'});
    end
    ...
    for point_idx=1:length( ERP_point{ERP_idx})
        ...
        rel_point(1,point_idx) = mean(bootstrp(200,@ICC,[data1(:,temp),data2(:,temp)],{'A-1'}));
        ci_point(:,point_idx) = bootci(200,@ICC,[data1(:,temp),data2(:,temp)],{'A-1'});
    end
    if ERP_idx==1
